...
public class TheoryGradingController extends BaseController {
    ...
    private void setupTable() {
        // Create table columns for MaterialFX TableView
        ...
        // Set column headers
        candidateNameCell.setHeaderText("Tên thí sinh");
        theoryScoreCell.setHeaderText("Điểm lý thuyết");
        statusCell.setHeaderText("Trạng thái");

        // Add columns to table
        tblCandidates.getTableColumns().addAll(candidateNameCell, theoryScoreCell, statusCell);

        // Enable sorting (MaterialFX compatible)
        candidateNameCell.setSortable(true);
        theoryScoreCell.setSortable(true);
        statusCell.setSortable(true);

        // Set default sort order
        tblCandidates.getTableColumns().get(0).setSortType(io.github.palexdev.materialfx.enums.SortState.ASCENDING);
    }
