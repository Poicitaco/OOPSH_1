...
public class MainLayoutController extends BaseController implements DashboardOperations {
    ...
    private Button createMenuButton(MenuItem item) {
        ...
        if (item.getIconLiteral() != null && !item.getIconLiteral().isEmpty()) {
            try {
                // Use Unicode icons as fallback since Ikonli might not be properly configured
                String iconText = getIconText(item.getIconLiteral());
                if (iconText != null) {
                    Label iconLabel = new Label(iconText);
                    iconLabel.setStyle("-fx-text-fill: white; -fx-font-size: 16px; -fx-font-family: 'Segoe UI Symbol';");
                    buttonContent.getChildren().add(iconLabel);
                }
            } catch (Exception e) {
                // Fallback to text-only if icon loading fails
                System.out.println("Warning: Could not load icon for " + item.getTitle());
            }
        }
    ...
    /**
     * Get Unicode icon text for Material Design icons
     */
    private String getIconText(String iconLiteral) {
        // Map Material Design icon literals to Unicode characters
        return switch (iconLiteral) {
            case "mdi2v-view-dashboard" -> "📊";
            case "mdi2c-chart-line" -> "📈";
            case "mdi2a-account-group" -> "👥";
            case "mdi2a-account-plus" -> "➕";
            case "mdi2s-shield-account" -> "🛡️";
            case "mdi2f-file-document-multiple" -> "📄";
            case "mdi2f-file-document-plus" -> "📝";
            case "mdi2f-file-document-edit" -> "✏️";
            case "mdi2c-calendar-clock" -> "📅";
            case "mdi2c-chart-bar" -> "📊";
            case "mdi2c-chart-pie" -> "🥧";
            case "mdi2c-currency-usd" -> "💰";
            case "mdi2c-trending-up" -> "📈";
            case "mdi2c-cog" -> "⚙️";
            case "mdi2c-cloud-upload" -> "☁️";
            case "mdi2f-file-document-outline" -> "📋";
            case "mdi2c-calendar" -> "📅";
            case "mdi2c-clock" -> "🕐";
            case "mdi2c-check-circle" -> "✅";
            case "mdi2c-book-open" -> "📖";
            case "mdi2c-school" -> "🎓";
            case "mdi2c-credit-card" -> "💳";
            case "mdi2c-history" -> "🕰️";
            case "mdi2c-star" -> "⭐";
            case "mdi2c-account" -> "👤";
            case "mdi2c-settings" -> "⚙️";
            case "mdi2c-help-circle" -> "❓";
            default -> "📄";
        };
    }
