...
public class DashboardStatsController extends BaseController {
    ...
    @FXML private Label lblTotalExaminers;
    @FXML private Label lblTotalAdmins;
    
    @FXML private Label lblTotalExams;
    @FXML private Label lblTotalSchedules;
    @FXML private Label lblUpcomingExams;
    @FXML private Label lblCompletedExams;
    
    @FXML private Label lblTotalRegistrations;
    @FXML private Label lblPendingRegistrations;
    @FXML private Label lblCompletedRegistrations;
    
    @FXML private Label lblTotalResults;
    @FXML private Label lblPassedResults;
    @FXML private Label lblFailedResults;
    @FXML private Label lblPassRate;
    
    @FXML private Label lblTotalRevenue;
    @FXML private Label lblMonthlyRevenue;
    @FXML private Label lblAverageExamFee;
    
    @FXML private ProgressBar pbPassRate;
    @FXML private ProgressBar pbRegistrationRate;
    @FXML private ProgressBar pbCompletionRate;
    
    @FXML private VBox vbRecentActivity;
    @FXML private VBox vbTopExamTypes;
    @FXML private VBox vbMonthlyStats;
    ...
    protected void setupEventHandlers() {
        ...
        // Platform.runLater(() -> {
        //     Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(30), event -> loadStatistics()));
    ...
    private void loadProgressBars() {
        ...
        List<Registration> registrations = registrationDAO.findAll();
        double registrationRate = candidates.isEmpty() ? 0.0 : 
            (double) registrations.size() / candidates.size();
        ...
        long completedRegistrations = registrations.stream()
            .filter(reg -> "COMPLETED".equals(reg.getStatus().toString()))
            .count();
        double completionRate = registrations.isEmpty() ? 0.0 : 
            (double) completedRegistrations / registrations.size();
