...
public class ExamResultsController extends BaseController {
    ...
    private void handleExport() {
        ...
        fileChooser.setInitialFileName("ket_qua_thi_" + java.time.LocalDate.now().format(java.time.format.DateTimeFormatter.ofPattern("yyyyMMdd")));
        ...
        if (file != null) {
            try {
                ...
                showInfo("Thành công", String.format("Đã xuất %d kết quả thi thành công!\nFile: %s", results.size(), file.getName()));
            } catch (Exception e) {
    ...
    private void exportToTXT(List<Result> results, java.io.File file) throws Exception {
        try (java.io.PrintWriter writer = new java.io.PrintWriter(file, "UTF-8")) {
            ...
            writer.println("Ngày xuất: " + java.time.LocalDate.now().format(java.time.format.DateTimeFormatter.ofPattern("dd/MM/yyyy")));
